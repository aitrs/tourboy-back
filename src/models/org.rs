...
impl Org {
    ...
    pub async fn all_orgs(
        ...
        let rq = format!(
            "
                ...
                {} {}
            ",
            if !req_filter.is_empty() { "AND" } else { "" },
            req_filter
        );
        let stmt = client.prepare_cached(rq.as_str()).await?;
    ...
    pub async fn tag_orgs(
        ...
        for id_org in orgs {
            ...
            client.query(&stmt2, &[&id_org, &id_user, &id_band]).await?;
        }
    ...
    pub async fn get_affected_users(
        &self,
        id_band: i32,
        id_orgs: Vec<i32>,
